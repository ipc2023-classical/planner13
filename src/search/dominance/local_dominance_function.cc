...
namespace dominance {
    ...
    bool applyPostSrc(const TransitionSystem &ts, int src,
                      std::function<bool(const Transition &t, LabelGroupID lg_id)> &&f) {
    ...
    LocalDominanceFunction<TCost>::LocalDominanceFunction(const TransitionSystem & _ts, int _ts_id,
                                                      int truncate_value,  std::shared_ptr<TauLabelInfo<TCost>> tau_labels_info)
            : ts(_ts), ts_id(_ts_id), truncate_value(truncate_value),
            tau_labels(tau_labels_info),
            max_relation_value(0),
            cancelled(false) {
    }

    template<typename T>
    void LocalDominanceFunction<T>::init_goal_respecting() {
    ...
    template<>
    void LocalDominanceFunction<IntEpsilon>::init_goal_respecting() {
        // TODO: Computing distances (possibly) again, they might already have been computed in task transformation
        Distances distances = task_transformation::Distances(ts);
        ...
        for (int s = 0; s < num_states; s++) {
            ...
            for (int t = 0; t < num_states; t++) {
                ...
                // } else {
                IntEpsilonSum rel =
                        IntEpsilonSum(distances.get_goal_distance(t)) - IntEpsilonSum(distances.get_goal_distance(s));
    ...
    template<typename T>
    T LocalDominanceFunction<T>::compare_transitions(int tr_s_target, LabelID tr_s_label,
    ...
    template<typename T>
    T LocalDominanceFunction<T>::compare_noop(int tr_s_target, LabelID tr_s_label,
    ...
    template<typename T>
    void LocalDominanceFunction<T>::cancel_simulation_computation() {
    ...
    template<typename T>
    vector<int> LocalDominanceFunction<T>::get_dangerous_labels() const {
    ...
    template<typename T>
    int LocalDominanceFunction<T>::update_pair_stable(const LabelDominanceFunction<T> &label_dominance,
                ...
                for (int t2: tau_distances.states_reachable_from(t)) {
                    ...
                    applyPostSrc(ts, t2, [&](const Transition &trt, LabelGroupID lg_id) {
                        for (int tr_t_label: ts.get_label_group(lg_id)) {
                            max_value = max(max_value,
                                            compare_transitions(trs.target, LabelID(tr_s_label), trt.target,
                                                                LabelID(tr_t_label),
    ...
    template<typename T>
    int LocalDominanceFunction<T>::update_pair(const LabelDominanceFunction<T> &label_dominance,
                                               const TauDistances<T> &tau_distances,
                                               int s, int t) {
    ...
    template<typename T>
    int LocalDominanceFunction<T>::update(const LabelDominanceFunction<T> &label_dominance, int max_time) {
        if (cancelled) {
    ...
    template<typename T>
    void LocalDominanceFunction<T>::dump(const vector <string> &names) const {
        cout << "SIMREL:" << endl;
    ...
    template<typename T>
    void LocalDominanceFunction<T>::dump() const {
        cout << "SIMREL:" << endl;
    ...
    template<typename T>
    bool LocalDominanceFunction<T>::has_dominance() const {
        for (int j = 0; size_t(j) < relation.size(); ++j) {
            for (int i = 0; size_t(i) < relation.size(); ++i) {
                if (i == j) continue;
    ...
    template<typename T>
    bool LocalDominanceFunction<T>::has_positive_dominance() const {
        for (int j = 0; size_t(j) < relation.size(); ++j) {
            for (int i = 0; size_t(i) < relation.size(); ++i) {
                if (i == j) continue;
                if (relation[i][j] >= 0) {
    ...
    template<typename T>
    void LocalDominanceFunction<T>::statistics() const {
        map<T, int> values;
        for (int j = 0; size_t(j) < relation.size(); ++j) {
            for (int i = 0; size_t(i) < relation.size(); ++i) {
                if (i == j) continue;
    ...
    template
    class LocalDominanceFunction<int>;

    template
    class LocalDominanceFunction<IntEpsilon>;
