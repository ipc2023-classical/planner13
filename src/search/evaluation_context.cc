...
EvaluationContext::get_preferred_operators(const task_representation::SearchTask & search_task,
                                           ...
                                           Evaluator *heur, ordered_set::OrderedSet<OperatorID> & preferred_operators) {
    get_result(heur).get_preferred_operators(cache.get_state(), search_task, applicable_operators, preferred_operators);
}
