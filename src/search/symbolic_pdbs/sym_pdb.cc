...
namespace symbolic {
	...
	SymStateSpaceManager(parent.getVars(), parent.getParams(), relevantVars), task(_task) {
	...
	for (int var : relevant_vars) {
	    abstract_ini.push_back(std::pair<int, int> (var, task->get_ts(var).get_init_state())); // Maybe use emplace_back
	}
