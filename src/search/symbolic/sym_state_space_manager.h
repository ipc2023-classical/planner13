...
public:
    ...
    inline bool isOriginal() const {
        return relevant_vars.size() == size_t(task->get_size());
    }
    ...
    int numVariablesAbstracted() const {
        return size_t(task->get_size()) - relevant_vars.size();
    }
...
//    }
    ...
    inline BDD getBDD(int variable, int value) const {
        return vars->sourceStateBDD(variable, value);
    }
    ...
    inline BDD bddVar(int val) const {
        return vars->bddVar(val);
    }
