...
namespace symbolic {
    ...
    public:
        //Constructor
        explicit OppositeFrontier(const std::shared_ptr<task_representation::FTSTask> &_task) : task(_task) {}
    ...
    class OppositeFrontierFixed : public OppositeFrontier {
        ...
        int hNotGoal;
    public:
        OppositeFrontierFixed(BDD g, const SymStateSpaceManager &mgr, const std::shared_ptr<task_representation::FTSTask> &_task);
        ...
        int getHNotClosed() const override {
            return hNotGoal;
        }
