...
//        DEBUG_MSG(cout << "Init perfect heuristic: " << endl;);
        if (closed_opposite) {
            ...
            if (fw) {
                perfectHeuristic = make_shared<OppositeFrontierFixed>(mgr->getGoal(), *mgr, task);
            } else {
                perfectHeuristic = make_shared<OppositeFrontierFixed>(mgr->getInitialState(), *mgr, task);
            }
        }
...
//            DEBUG_MSG(
//                    cout << "MAX STEP NODES CHANGED TO: " << p.maxStepNodes << " after taking " << sTime() << " seconds"
//                         << endl;);
