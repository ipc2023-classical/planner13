...
protected:
    ...
    int update_pair(const NumericLabelRelation<T> &label_dominance,
                    const TauDistances<T> &tau_distances,
                    int s, int t);

    int update_pair_stable(const NumericLabelRelation<T> &label_dominance,
                           const TauDistances<T> &tau_distances,
                           int s, int t);

public:
    NumericSimulationRelation(const TransitionSystem& ts, int ts_id, int truncate_value,
                              std::shared_ptr<TauLabelManager<T>> tau_labels_mgr);

    void init_goal_respecting();

    int update(const NumericLabelRelation<T> &label_dominance,
               int max_time);
