...
def main(revisions=None):
    ...
    configs = {
        ...
        # IssueConfig('astar-masdfpbisim50k-transform-atomic-labelreduction', ["--transform", "transform_merge_and_shrink(label_reduction=exact(atomic_fts=true,before_shrinking=true,before_merging=false),run_main_loop=false)", "--search", "astar(merge_and_shrink(shrink_strategy=mas_shrink_bisimulation(greedy=false),merge_strategy=mas_merge_stateless(merge_selector=mas_score_based_filtering(scoring_functions=[mas_goal_relevance,mas_dfp,mas_total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=mas_exact(before_shrinking=true,before_merging=false),max_states=50000,threshold_before_merge=1))"]),
        # IssueConfig('astar-masdfpbisim50k-transform-atomic-bisim', ["--transform", "transform_merge_and_shrink(shrink_strategy=shrink_bisimulation(greedy=false),shrink_atomic_fts=true,run_main_loop=false)", "--search", "astar(merge_and_shrink(shrink_strategy=mas_shrink_bisimulation(greedy=false),merge_strategy=mas_merge_stateless(merge_selector=mas_score_based_filtering(scoring_functions=[mas_goal_relevance,mas_dfp,mas_total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=mas_exact(before_shrinking=true,before_merging=false),max_states=50000,threshold_before_merge=1))"]),
        ...
        # IssueConfig('astar-masmiasmbisim50k-transform-atomic-labelreduction', ["--transform", "transform_merge_and_shrink(label_reduction=exact(atomic_fts=true,before_shrinking=true,before_merging=false),run_main_loop=false)", "--search", "astar(merge_and_shrink(shrink_strategy=mas_shrink_bisimulation(greedy=false),merge_strategy=mas_merge_stateless(merge_selector=mas_score_based_filtering(scoring_functions=[mas_sf_miasm(shrink_strategy=mas_shrink_bisimulation(greedy=false),max_states=50000,threshold_before_merge=1),mas_total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=mas_exact(before_shrinking=true,before_merging=false),max_states=50000,threshold_before_merge=1))"]),
        # IssueConfig('astar-masmiasmbisim50k-transform-atomic-bisim', ["--transform", "transform_merge_and_shrink(shrink_strategy=shrink_bisimulation(greedy=false),shrink_atomic_fts=true,run_main_loop=false)", "--search", "astar(merge_and_shrink(shrink_strategy=mas_shrink_bisimulation(greedy=false),merge_strategy=mas_merge_stateless(merge_selector=mas_score_based_filtering(scoring_functions=[mas_sf_miasm(shrink_strategy=mas_shrink_bisimulation(greedy=false),max_states=50000,threshold_before_merge=1),mas_total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=mas_exact(before_shrinking=true,before_merging=false),max_states=50000,threshold_before_merge=1))"]),
    ...
    exp.add_absolute_report_step(attributes=attributes, filter_algorithm=[
        ...
        '{}-astar-masdfpbisim50k-transform-atomic-bisim'.format(REVISION),
        ...
        '{}-astar-masmiasmbisim50k-transform-atomic-bisim'.format(REVISION),
    ...
    exp.add_report(
        ComparativeReport(
            algorithm_pairs=[
                ...
                # ('{}-astar-masdfpbisim50k'.format(BASELINE_REV), '{}-astar-masdfpbisim50k-transform-atomic-bisim'.format(REVISION)),
                ...
                # ('{}-astar-masmiasmbisim50k'.format(BASELINE_REV), '{}-astar-masmiasmbisim50k-transform-atomic-bisim'.format(REVISION)),
    ...
    exp.add_report(
        ComparativeReport(
            algorithm_pairs=[
                # ('{}-astar-masdfpbisim50k-atomic'.format(REVISION), '{}-astar-masdfpbisim50k-transform-atomic-labelreduction'.format(REVISION)),
                # ('{}-astar-masdfpbisim50k-atomic'.format(REVISION), '{}-astar-masdfpbisim50k-transform-atomic-bisim'.format(REVISION)),
                ('{}-astar-masdfpbisim50k-atomic'.format(REVISION), '{}-astar-masdfpbisim50k-transform-atomic-bisim-labelreduction'.format(REVISION)),
                # ('{}-astar-masmiasmbisim50k-atomic'.format(REVISION), '{}-astar-masmiasmbisim50k-transform-atomic-labelreduction'.format(REVISION)),
                # ('{}-astar-masmiasmbisim50k-atomic'.format(REVISION), '{}-astar-masmiasmbisim50k-transform-atomic-bisim'.format(REVISION)),
