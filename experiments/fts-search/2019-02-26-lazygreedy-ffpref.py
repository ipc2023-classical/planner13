...
def main(revisions=None):
    ...
    configs = {
        IssueConfig('lazy-ffpref-transform-atomic', ["--heuristic", "hff=ff(cost_type=one)", "--search", "lazy_greedy([hff], preferred=[hff],cost_type=one)"]),
        IssueConfig('lazy-ffpref-transform-atomic-labelreduction', ['--transform', 'transform_merge_and_shrink(label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=false,run_main_loop=false,max_time=900,prune_transitions_from_goal=true,prune_transitions_from_goal=true)', "--heuristic", "hff=ff(cost_type=one)", "--search", "lazy_greedy([hff], preferred=[hff],cost_type=one)"]),
        IssueConfig('lazy-ffpref-transform-atomic-weakbisim-labelreduction', ['--transform', 'transform_merge_and_shrink(shrink_strategy=shrink_weak_bisimulation(ignore_irrelevant_tau_groups=false),label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=false,max_time=900,prune_transitions_from_goal=true,prune_transitions_from_goal=true)', "--heuristic", "hff=ff(cost_type=one)", "--search", "lazy_greedy([hff], preferred=[hff],cost_type=one)"]),
        IssueConfig('lazy-ffpref-transform-full-weakbisim-labelreduction-dfp100-t900', ["--transform", 'transform_merge_and_shrink(shrink_strategy=shrink_weak_bisimulation(ignore_irrelevant_tau_groups=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(100),goal_relevance,dfp,total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900,prune_transitions_from_goal=true,prune_transitions_from_goal=true)', "--heuristic", "hff=ff(cost_type=one)", "--search", "lazy_greedy([hff], preferred=[hff],cost_type=one)"]),
    ...
    exp.add_absolute_report_step(attributes=attributes, filter_algorithm=[
        '{}-lazy-ffpref-transform-atomic'.format(REVISION),
    ...
    exp.add_report(
        ComparativeReport(
            algorithm_pairs=[
                ('{}-lazy-ffpref'.format(BASELINE_REV), '{}-lazy-ffpref-transform-atomic'.format(REVISION)),
