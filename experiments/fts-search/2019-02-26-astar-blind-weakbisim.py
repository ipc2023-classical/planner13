...
def main(revisions=None):
    ...
    configs = {
        IssueConfig('astar-blind-transform-atomic', ["--search", "astar(blind,cost_type=one)"]),
        IssueConfig('astar-blind-transform-atomic-labelreduction', ["--transform", "transform_merge_and_shrink(label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=false,run_main_loop=false)", "--search", "astar(blind,cost_type=one)"]),
        IssueConfig('astar-blind-transform-atomic-weakbisim-labelreduction', ["--transform", "transform_merge_and_shrink(shrink_strategy=shrink_weak_bisimulation(ignore_irrelevant_tau_groups=false),label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=false,prune_transitions_from_goal=true,prune_transitions_from_goal=true)", "--search", "astar(blind,cost_type=one)"]),
    ...
    exp.add_absolute_report_step(attributes=attributes, filter_algorithm=[
        '{}-astar-blind-transform-atomic'.format(REVISION),
    ...
    exp.add_fetcher('data/2019-02-18-astar-baseline-eval',merge=True)

    outfile = os.path.join(exp.eval_dir, get_experiment_name() + '-regular-vs-fts.html')
    exp.add_report(
        ComparativeReport(
            algorithm_pairs=[
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-atomic'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-atomic-labelreduction'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-atomic-weakbisim-labelreduction'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-weakbisim-labelreduction-dfp100-t900'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-weakbisim-labelreduction-dfp1000-t900'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-weakbisim-labelreduction-dfp10000-t900'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-weakbisim-labelreduction-miasm100-t900'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-weakbisim-labelreduction-miasm1000-t900'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-weakbisim-labelreduction-miasm10000-t900'.format(REVISION)),
            ],
            format='html',
