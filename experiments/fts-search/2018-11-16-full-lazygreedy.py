...
def main(revisions=None):
    ...
    configs = {
        ...
        IssueConfig('lazy-ff-transform-full-own-labelreduction-dfp1k-t900', ["--transform", "transform_merge_and_shrink(shrink_strategy=own_labels,merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(1000),goal_relevance,dfp,total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900)", "--search", "lazy_greedy([ff()])"]),
        IssueConfig('lazy-ff-transform-full-own-labelreduction-miasm1k-t900', ["--transform", "transform_merge_and_shrink(shrink_strategy=own_labels,merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(1000),sf_miasm(shrink_strategy=shrink_bisimulation(greedy=false),max_states=1000,threshold_before_merge=1),total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900)", "--search", "lazy_greedy([ff()])"]),
        IssueConfig('lazy-ff-transform-full-own-labelreduction-miasm1kown-t900', ["--transform", "transform_merge_and_shrink(shrink_strategy=own_labels,merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(1000),sf_miasm(shrink_strategy=own_labels,max_states=1000,threshold_before_merge=1),total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900)", "--search", "lazy_greedy([ff()])"]),
    }
    ...
    exp.add_absolute_report_step(attributes=attributes, filter_algorithm=[
        ...
        '{}-lazy-ff-transform-full-own-labelreduction-dfp1k-t900'.format(REVISION),
        '{}-lazy-ff-transform-full-own-labelreduction-miasm1k-t900'.format(REVISION),
        '{}-lazy-ff-transform-full-own-labelreduction-miasm1kown-t900'.format(REVISION),
    ])
    ...
    exp.add_report(
        ComparativeReport(
            algorithm_pairs=[
                ...
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-transform-full-own-labelreduction-dfp1k-t900'.format(REVISION)),
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-transform-full-own-labelreduction-miasm1k-t900'.format(REVISION)),
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-transform-full-own-labelreduction-miasm1kown-t900'.format(REVISION)),
            ],
            format='html',
