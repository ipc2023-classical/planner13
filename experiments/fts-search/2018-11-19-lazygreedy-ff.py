...
def main(revisions=None):
    ...
    configs = {
        ...
        IssueConfig('lazy-ff-transform-full-bisimown-labelreduction-miasm1000-t900', ["--transform", 'transform_merge_and_shrink(shrink_strategy=shrink_own_bisimulation(),merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(1000),sf_miasm(shrink_strategy=shrink_own_bisimulation,threshold_before_merge=1,max_states=1000),total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900,cost_type=one)', '--search',  'lazy_greedy([ff(cost_type=one)], cost_type=one)']),
    ...
    exp.add_absolute_report_step(attributes=attributes, filter_algorithm=[
        ...
        '{}-lazy-ff-transform-full-bisimown-labelreduction-dfp100-t900'.format(REVISION),
        '{}-lazy-ff-transform-full-bisimown-labelreduction-dfp1000-t900'.format(REVISION),
        '{}-lazy-ff-transform-full-bisimown-labelreduction-dfp10000-t900'.format(REVISION),
    ...
    exp.add_report(
        ComparativeReport(
            algorithm_pairs=[
                ...
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-transform-full-bisimown-labelreduction-dfp100-t900'.format(REVISION)),
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-transform-full-bisimown-labelreduction-dfp1000-t900'.format(REVISION)),
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-transform-full-bisimown-labelreduction-dfp10000-t900'.format(REVISION)),
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-transform-full-bisimown-labelreduction-miasm100-t900'.format(REVISION)),
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-transform-full-bisimown-labelreduction-miasm1000-t900'.format(REVISION)),
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-transform-full-bisimown-labelreduction-miasm10000-t900'.format(REVISION)),
            ],
            format='html',
