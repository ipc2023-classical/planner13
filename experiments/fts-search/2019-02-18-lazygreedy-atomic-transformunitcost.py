...
def main(revisions=None):
    ...
    configs = {
        IssueConfig('lazy-ff-atomic-transformunitcost', ["--transform", "cost(cost_type=one)", "--search", 'lazy_greedy([ff(cost_type=one)], cost_type=one)']),
        IssueConfig('lazy-ffpref-atomic-transformunitcost', ["--transform", "cost(cost_type=one)", "--heuristic", "hff=ff(cost_type=one)", "--search", "lazy_greedy([hff], preferred=[hff], cost_type=one)"]),
    }
    ...
    exp.add_absolute_report_step(attributes=attributes, filter_algorithm=[
        '{}-lazy-ff-atomic-transformunitcost'.format(REVISION),
        '{}-lazy-ffpref-atomic-transformunitcost'.format(REVISION),
