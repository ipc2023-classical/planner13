...
def main(revisions=None):
    ...
    configs = {
        ...
        IssueConfig('lazy-ff-transform-atomic-own-labelreduction', ["--transform", "transform_merge_and_shrink(shrink_strategy=own_labels,label_reduction=exact(atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=false)", "--search", "lazy_greedy([ff()])"]),
    ...
    exp.add_absolute_report_step(attributes=attributes, filter_algorithm=[
        ...
        '{}-lazy-ff-transform-atomic-own-labelreduction'.format(REVISION),
    ...
    outfile = os.path.join(exp.eval_dir, get_experiment_name() + '-regular-vs-fts-and-atomic-vs-transform.html')
    exp.add_report(
        ComparativeReport(
            algorithm_pairs=[
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-atomic'.format(REVISION)),
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-transform-atomic-own-labelreduction'.format(REVISION)),
                ('{}-lazy-ff'.format(BASELINE_REV), '{}-lazy-ff-transform-atomic-bisim-labelreduction'.format(REVISION)),

                ('{}-lazy-ff-atomic'.format(REVISION), '{}-lazy-ff-transform-atomic-own-labelreduction'.format(REVISION)),
