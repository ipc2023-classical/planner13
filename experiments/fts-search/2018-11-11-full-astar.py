...
def main(revisions=None):
    ...
    configs = {
        ...
        IssueConfig('astar-hmax-transform-full-bisim-labelreduction-miasm50k-t900', ["--transform", "transform_merge_and_shrink(shrink_strategy=shrink_bisimulation(greedy=false),merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(50000),sf_miasm(shrink_strategy=shrink_bisimulation(greedy=false),max_states=50000,threshold_before_merge=1),total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900)", "--search", "astar(hmax)"]),
    }
    ...
    exp.add_absolute_report_step(attributes=attributes, filter_algorithm=[
        '{}-astar-blind-transform-full-bisim-labelreduction-dfp50k-t900'.format(REVISION),
        '{}-astar-blind-transform-full-bisim-labelreduction-miasm50k-t900'.format(REVISION),
        '{}-astar-hmax-transform-full-bisim-labelreduction-dfp50k-t900'.format(REVISION),
        '{}-astar-hmax-transform-full-bisim-labelreduction-miasm50k-t900'.format(REVISION),
    ])
    ...
    exp.add_report(
        ComparativeReport(
            algorithm_pairs=[
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-bisim-labelreduction-dfp50k-t900'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-bisim-labelreduction-miasm50k-t900'.format(REVISION)),
                ('{}-astar-hmax'.format(BASELINE_REV), '{}-astar-hmax-transform-full-bisim-labelreduction-dfp50k-t900'.format(REVISION)),
                ('{}-astar-hmax'.format(BASELINE_REV), '{}-astar-hmax-transform-full-bisim-labelreduction-miasm50k-t900'.format(REVISION)),
            ],
            format='html',
