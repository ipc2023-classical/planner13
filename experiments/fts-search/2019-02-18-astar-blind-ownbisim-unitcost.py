...
def main(revisions=None):
    ...
    configs = {
        IssueConfig('astar-blind-unitcost-transform-atomic-ownbisim-labelreduction', ["--transform", "transform_merge_and_shrink(shrink_strategy=shrink_own_bisimulation,label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=false,cost_type=one)", "--search", "astar(blind, cost_type=one)"]),
        IssueConfig('astar-blind-unitcost-transform-full-ownbisim-labelreduction-dfp100-t900', ["--transform", "transform_merge_and_shrink(shrink_strategy=shrink_own_bisimulation,merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(100),goal_relevance,dfp,total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900,cost_type=one)", "--search", "astar(blind, cost_type=one)"]),
        IssueConfig('astar-blind-unitcost-transform-full-ownbisim-labelreduction-dfp1000-t900', ["--transform", "transform_merge_and_shrink(shrink_strategy=shrink_own_bisimulation,merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(1000),goal_relevance,dfp,total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900,cost_type=one)", "--search", "astar(blind, cost_type=one)"]),
        IssueConfig('astar-blind-unitcost-transform-full-ownbisim-labelreduction-dfp10000-t900', ["--transform", "transform_merge_and_shrink(shrink_strategy=shrink_own_bisimulation,merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(10000),goal_relevance,dfp,total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900,cost_type=one)", "--search", "astar(blind, cost_type=one)"]),
        IssueConfig('astar-blind-unitcost-transform-full-ownbisim-labelreduction-miasm100-t900', ["--transform", "transform_merge_and_shrink(shrink_strategy=shrink_own_bisimulation,merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(100),sf_miasm(shrink_strategy=shrink_own_bisimulation,max_states=100,threshold_before_merge=1),total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900,cost_type=one)", "--search", "astar(blind, cost_type=one)"]),
        IssueConfig('astar-blind-unitcost-transform-full-ownbisim-labelreduction-miasm1000-t900', ["--transform", "transform_merge_and_shrink(shrink_strategy=shrink_own_bisimulation,merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(1000),sf_miasm(shrink_strategy=shrink_own_bisimulation,max_states=1000,threshold_before_merge=1),total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900,cost_type=one)", "--search", "astar(blind, cost_type=one)"]),
        IssueConfig('astar-blind-unitcost-transform-full-ownbisim-labelreduction-miasm10000-t900', ["--transform", "transform_merge_and_shrink(shrink_strategy=shrink_own_bisimulation,merge_strategy=merge_stateless(merge_selector=score_based_filtering(scoring_functions=[product_size(10000),sf_miasm(shrink_strategy=shrink_own_bisimulation,max_states=10000,threshold_before_merge=1),total_order(atomic_ts_order=reverse_level,product_ts_order=new_to_old,atomic_before_product=false)])),label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=true,max_time=900,cost_type=one)", "--search", "astar(blind, cost_type=one)"]),
    }
    ...
    exp.add_absolute_report_step(attributes=attributes, filter_algorithm=[
        '{}-astar-blind-unitcost-transform-atomic-ownbisim-labelreduction'.format(REVISION),
        '{}-astar-blind-unitcost-transform-full-ownbisim-labelreduction-dfp100-t900'.format(REVISION),
        '{}-astar-blind-unitcost-transform-full-ownbisim-labelreduction-dfp1000-t900'.format(REVISION),
        '{}-astar-blind-unitcost-transform-full-ownbisim-labelreduction-dfp10000-t900'.format(REVISION),
        '{}-astar-blind-unitcost-transform-full-ownbisim-labelreduction-miasm100-t900'.format(REVISION),
        '{}-astar-blind-unitcost-transform-full-ownbisim-labelreduction-miasm1000-t900'.format(REVISION),
        '{}-astar-blind-unitcost-transform-full-ownbisim-labelreduction-miasm10000-t900'.format(REVISION),
    ])
    ...
    exp.add_report(
        ComparativeReport(
            algorithm_pairs=[
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-atomic-ownbisim-labelreduction'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-ownbisim-labelreduction-dfp100-t900'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-ownbisim-labelreduction-dfp1000-t900'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-ownbisim-labelreduction-dfp10000-t900'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-ownbisim-labelreduction-miasm100-t900'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-ownbisim-labelreduction-miasm1000-t900'.format(REVISION)),
                ('{}-astar-blind'.format(BASELINE_REV), '{}-astar-blind-transform-full-ownbisim-labelreduction-miasm10000-t900'.format(REVISION)),
            ],
            format='html',
