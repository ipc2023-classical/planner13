...
def main(revisions=None):
    ...
    configs = {
        IssueConfig('lazy-ff-transformunitcost-atomic-weakbisim-labelreduction', ['--transform', 'transform_merge_and_shrink(shrink_strategy=shrink_weak_bisimulation(ignore_irrelevant_tau_groups=true, apply_haslum_rule=true),label_reduction=exact(max_time=300,atomic_fts=true,before_shrinking=true,before_merging=false),shrink_atomic_fts=true,run_main_loop=false,max_time=900,cost_type=one,prune_transitions_from_goal=true,prune_transitions_from_goal=true)', '--search',  'lazy_greedy([ff(cost_type=one)], cost_type=one)']),
